    ...
    def paintEvent(self, e):
        ...
        if self.number > 0:
            ...
            #Draw candidate if > 0
                if self.candidate > 0:
                    ...
                    p.setFont(f)
                    p.drawText(r, Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignVCenter, str(self.candidate))                      
        ...
        if self.an_fontsize > 0 and len(self.a_numbers) > 0:
            ...
            if len(self.a_numbers) < 6:
                ...
                p.drawText(r, Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignBottom, ' '.join(sl))
            else:
                st = [str(n) for n in self.a_numbers[:int(len(self.a_numbers)/2)]]
                p.drawText(r, Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignTop, ' '.join(st))
                sb = [str(n) for n in self.a_numbers[int(len(self.a_numbers)/2):]]
                p.drawText(r, Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignBottom, ' '.join(sb))
...
class Grid(QtWidgets.QFrame):
    ...
    def resizeEvent(self, e):
        ...
        new_size.scale(e.size(), QtCore.Qt.AspectRatioMode.KeepAspectRatio)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.grid_frame.setFocusPolicy(QtCore.Qt.FocusPolicy.StrongFocus)
        ...
        self.gen_display.setFrameStyle(QtWidgets.QFrame.Shape.Panel|QtWidgets.QFrame.Shadow.Raised)
        ...
        load_action = QtGui.QAction("&Open", self)
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() == Qt.Key.Key_Right and self.active_x < 8:
            self.toggle_activate_tile(self.active_x+1, self.active_y)
        if e.key() == Qt.Key.Key_Left and self.active_x > 0:
            self.toggle_activate_tile(self.active_x-1, self.active_y)            
        if e.key() == Qt.Key.Key_Up and self.active_y > 0:
            self.toggle_activate_tile(self.active_x, self.active_y-1)
        if e.key() == Qt.Key.Key_Down and self.active_y < 8:
            self.toggle_activate_tile(self.active_x, self.active_y+1)
        ...
        if e.key() == Qt.Key.Key_Backspace and self.tile_active:
            self.clear_number(self.active_x, self.active_y)
        #Enter deactivates tile
        if e.key() == Qt.Key.Key_Return and self.tile_active:
             self.toggle_activate_tile(self.active_x, self.active_y) 
        if e.key() == Qt.Key.Key_L and self.tile_active:
    ...
    def show_file_dialog(self, save = False):
        ...
        if save: 
            dialog.setFileMode(QFileDialog.FileMode.AnyFile) #Select also non-existing files
            dialog.setAcceptMode(QFileDialog.AcceptMode.AcceptSave)        
        else:
            dialog.setFileMode(QFileDialog.FileMode.ExistingFile) #Select existing file
